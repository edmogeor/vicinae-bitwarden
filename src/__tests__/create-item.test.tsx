...
const {
  ...
  const MockForm = Object.assign(
    function FormWrapper(props: { children: React.ReactNode; actions?: React.ReactNode; isLoading?: boolean }) {
  ...
  const MockActionSubmitForm = vi.fn(
    ({ title, onSubmit }: { title: string; onSubmit: (values: Record<string, unknown>) => void }) => {
      _handler = onSubmit;
