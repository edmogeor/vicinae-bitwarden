...
describe("login", () => {
  ...
  it("calls bw config server then bw login --apikey with env vars", async () => {
    ...
    expect(mockExecFile).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({ env: expect.objectContaining({ BW_CLIENTID: "user.123", BW_CLIENTSECRET: "secret" }) }),
    );
    expect(mockExecFile).toHaveBeenNthCalledWith(
      2,
      "bw",
...
describe("status", () => {
  it("calls bw status and returns parsed result", async () => {
    const statusObj = { serverUrl: null, lastSync: null, userEmail: "a@b.com", userId: "x", status: "unlocked" as const };
